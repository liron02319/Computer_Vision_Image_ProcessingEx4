...
def disparityHelp(img_l: np.ndarray,img_r: np.ndarray, disp_range: (int, int), k_size: int) -> (np.ndarray,np.ndarray,np.ndarray,np.ndarray,np.ndarray):
    """
    help function for disparitySSD and disparityNC functions
...
def disparitySSD(img_l: np.ndarray, img_r: np.ndarray, disp_range: (int, int), k_size: int) -> np.ndarray:
    ...
    disparity_map, left, right, height, width, normalize_left, normalize_right = disparityHelp(img_l, img_r, disp_range, k_size)
...
def disparityNC(img_l: np.ndarray, img_r: np.ndarray, disp_range: int, k_size: int) -> np.ndarray:
    ...
    return: Disparity map, disp_map.shape = Left.shape
    """
    disparity_map, left, right, height, width, normalize_left, normalize_right = disparityHelp(img_l, img_r, disp_range,
                                                                                           k_size)
...
def computeHomography(src_pnt: np.ndarray, dst_pnt: np.ndarray) -> (np.ndarray, float):
    ...
    err = np.sqrt(np.sum(differ ** 2))  # get the err

    return homograph, err


#help function for warpImag
...
def warpImag(src_img: np.ndarray, dst_img: np.ndarray) -> None:
    ...
    TL_TR_slope, TL_TR_b = get_equation((dst_p[tl][0], dst_p[tl][1]), (dst_p[tr][0], dst_p[tr][1]))
    BL_BR_slope, BL_BR_b = get_equation((dst_p[bl][0], dst_p[bl][1]), (dst_p[br][0], dst_p[br][1]))
    TL_BL_slope, TL_BL_b = get_equation((dst_p[tl][0], dst_p[tl][1]), (dst_p[bl][0], dst_p[bl][1]))
    TR_BR_slope, TR_BR_b = get_equation((dst_p[br][0], dst_p[br][1]), (dst_p[tr][0], dst_p[tr][1]))

    # run on the image
    for x in range(dst_img.shape[0]):
        for y in range(dst_img.shape[1]):
            if minimumX <= y <= maximumX and minimumY <= x <= maximumY:  # check
                if TL_TR_slope * y + TL_TR_b <= x <= BL_BR_slope * y + BL_BR_b:
                    if TL_BL_slope != 0 and TR_BR_slope != 0:
                        if (x - TL_BL_b) / TL_BL_slope <= y <= ( x - TR_BR_b) / TR_BR_slope:
                            ...
                            mask[x][y][1] = 1
                            mask[x][y][2] = 1
                    elif TL_BL_slope != 0 and TR_BR_slope == 0:  # check
    ...
    out = dst_img * (1 - mask) + srcOutput * (mask)   # link the images
    plt.imshow(out)
    plt.show()
